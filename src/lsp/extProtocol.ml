...
module Notification = struct
  ...
  module Client = struct
    ...
    let t_of_jsonrpc (JsonRpc.Notification.{ method_; params } as notif) =
      match method_ with
      | "ec/interpretToPoint" -> InterpretToPoint InterpretToPointParams.(t_of_yojson params)
      | "ec/stepBackward" -> StepBackward StepBackwardParams.(t_of_yojson params)
      | "ec/stepForward" -> StepForward StepForwardParams.(t_of_yojson params)
      | "ec/interpretToEnd" -> InterpretToEnd InterpretToEndParams.(t_of_yojson params)
...
module Request = struct
  ...
  let t_of_jsonrpc (JsonRpc.Request.{ id; method_; params } as req) =
    match method_ with
    | "ec/resetCoq" -> Ext (id, Reset ResetParams.(t_of_yojson params))
    | "ec/search" -> Ext (id, Search SearchParams.(t_of_yojson params))
    | "ec/about" -> Ext (id, About AboutParams.(t_of_yojson params))
    | "ec/check" -> Ext (id, Check CheckParams.(t_of_yojson params))
    | "ec/locate" -> Ext (id, Locate LocateParams.(t_of_yojson params))
    | "ec/print" -> Ext (id, Print PrintParams.(t_of_yojson params))
    | "ec/documentState" -> Ext (id, DocumentState DocumentStateParams.(t_of_yojson params))
    | _ ->
